...
- **Highest Priority**: Hidden words, anagrams, and acrostics. Once an LLM identifies the operational mechanism and fodder, the algorithmic solver generates a highly constrained, reliable candidate list. 
...
- **LLM**: Deep semantic matching. Scores each candidate generated by the algorithm to see if it fits both definition-like phrases.

### 3. Hidden Clues
**Very Algorithmic-Friendly**
- **LLM**: Spots the oblique hidden-indicator and verifies the definition matching at the end.
...
- **LLM**: Proposes parses (which word is the definition, which components are abbreviations vs synonyms).
- **Algorithm**: Uses tables of abbreviations and crosswordese to build candidates by concatenation.
- *Risk*: Combinatorial explosion. The LLM must prune bad components early, and the algorithmic engine requires optimized data structures (like Tries) to abandon dead-end combinations.

### 5. Start, Middle, and End (Acrostics)
**Mostly Algorithmic**
- **LLM**: Detects the operation ("initially", "heart of") and the target span.
- **Algorithm**: Mechanical transforms (first letters, odd letters, etc.). Generation is trivial once parsed.

### 6. Container and Contents
**Hybrid**
- **LLM**: Works out which chunk goes inside which, and identifies deletions or abbreviations before insertion.
...
- **LLM**: Judges plausibility, selects intended synonyms, and accounts for accent-dependent loose matches ("we hear").
...
- **LLM**: Detects which complex operation applies (e.g., "endless" = delete last letter).
